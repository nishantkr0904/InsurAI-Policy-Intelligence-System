...
const MOCK_ISSUES: ComplianceIssue[] = [
  {
    ...
    description: "PII data retention policy exceeds 90 days",
    status: "open",
  },
  {
    id: "CPL-002",
    severity: "warning",
    rule: "SOC 2 CC6.1",
    description: "Access logs not retained for minimum 12 months",
    status: "open",
  },
  {
    id: "CPL-003",
    severity: "info",
    rule: "ISO 27001 A.8.2",
    description: "Data classification labels missing on 14 policy documents",
    status: "acknowledged",
  },
  {
    id: "CPL-004",
    severity: "critical",
    rule: "NAIC Model #674",
    description: "Claims denial letters missing required regulatory language",
    status: "open",
  },
  {
    id: "CPL-005",
    severity: "warning",
    rule: "State Insurance Code §428",
    description: "Policy renewal notices sent 25 days before expiry (minimum: 30 days)",
    status: "resolved",
  },
];
...
export default function CompliancePage() {
  ...
  const score = calcScore(MOCK_ISSUES);
  const openCritical = MOCK_ISSUES.filter((i) => i.severity === "critical" && i.status !== "resolved").length;
          ...
          <div className="space-y-2">
            ...
            {ran && (
              <p className="text-xs flex items-center gap-1.5" style={{ color: "var(--success)" }}>
                ...
                Check complete — {MOCK_ISSUES.length} issues found
              </p>
            )}
          </div>
        </div>
